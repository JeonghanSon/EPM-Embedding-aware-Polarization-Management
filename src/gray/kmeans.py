...
def _load_best_z(dataset: str, seed: int) -> np.ndarray:
    ...
    if not isinstance(z, torch.Tensor) or z.ndim != 2:
        raise TypeError(f"{dataset}: best_z.pt must be 2D Tensor (got {type(z)}, shape={getattr(z, 'shape', None)})")
    return z.detach().cpu().float().numpy()


def _save_nodewise(labels: np.ndarray, out_path: Path):
    df = pd.DataFrame({"node_id": np.arange(len(labels), dtype=int), "community": labels.astype(int)})
...
def run(dataset: str, seed: int, random_state: int = 42):
    k = _load_k(dataset)
    X = _load_best_z(dataset, seed)

    km = KMeans(n_clusters=k, random_state=random_state, n_init=10)
    labels = km.fit_predict(X)

    out_dir = RESULTS_GRAY / dataset / f"seed{seed}" / "aug"
    out_dir.mkdir(parents=True, exist_ok=True)

    _save_nodewise(labels, out_dir / "kmeans_node.csv")
    _save_groupwise(labels, out_dir / "kmeans_community.csv")

    print(f"✅ kmeans saved: dataset={dataset} seed={seed} k={k} -> {out_dir}")
...
def main():
    ...
    args = p.parse_args()
    run(args.dataset, args.seed, args.random_state)
